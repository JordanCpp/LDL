...
uint32_t UtfConverter::decodeUTF8Char(const char* utf8_str, size_t& pos, size_t max_len) 
{
...
void UtfConverter::encodeUTF8Char(uint32_t code_point, Vector<char>& output) 
{
    if (code_point <= 0x7F) 
    {
...
bool UtfConverter::convertToUTF16(const char* utf8_str)
{
    utf16_buffer.clear();
...
bool UtfConverter::convertToUTF8(const uint16_t* utf16_str, size_t utf16_len) 
{
    utf8_buffer.clear();
...
bool UtfConverter::convertToUTF8(const Vector<uint16_t>& utf16_data) 
{
...
const uint16_t* UtfConverter::getUTF16Data() const 
{
    return utf16_buffer.empty() ? NULL : &utf16_buffer[0];
}

const char* UtfConverter::getUTF8Data() const
{
    return utf8_buffer.empty() ? NULL : &utf8_buffer[0];
}

const Vector<uint16_t>& UtfConverter::getUTF16Vector() const
{
    return utf16_buffer;
}

const Vector<char>& UtfConverter::getUTF8Vector() const 
{
    return utf8_buffer;
}

void UtfConverter::clear() 
{
    utf16_buffer.clear();
    utf8_buffer.clear();
}

bool UtfConverter::hasUTF16() const
{
    return !utf16_buffer.empty();
}

bool UtfConverter::hasUTF8() const 
{
