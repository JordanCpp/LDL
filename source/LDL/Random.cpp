...
void LDL_Random::Seed(size_t seed)
{
    _seed = seed ? seed : 1;
}

size_t LDL_Random::GetSeed() const
{
    return _seed;
}

int LDL_Random::Range(int min, int max)
{
    if (min >= max)
    {
        return min;
...
size_t LDL_Random::Range(size_t min, size_t max)
{
    if (min >= max)
    {
        return min;
...
float LDL_Random::Range(float min, float max)
{
    if (min >= max)
    {
        return min;
