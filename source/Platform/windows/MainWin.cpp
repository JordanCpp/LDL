...
LRESULT CALLBACK LDL_MainWindow::Handler(UINT Message, WPARAM WParam, LPARAM LParam)
{
	...
	case WM_SIZE:
		...
		_BaseWindow.Size(LDL_Vec2i((int)event.Resize.Width, (int)event.Resize.Height));
		_Eventer.Push(event);
		break;
...
{
	_HWND = CreateWindow(AppName, "", _Style, (int)_BaseWindow.Pos().x, (int)_BaseWindow.Pos().y, _Rect.right - _Rect.left, _Rect.bottom - _Rect.top, 0, 0, _HINSTANCE, 0);
