...
{
	return render->_Render.Size().PosX();
}

size_t LDL_RenderGetSizeY(LDL_Render* render)
{
	return render->_Render.Size().PosY();
}
...
{
	render->_Render.Draw((Texture*)texture, Point2u(x, y));
}

void LDL_RenderDrawTexture2(LDL_Render* render, LDL_Texture* texture, size_t x, size_t y, size_t w, size_t h)
{
	render->_Render.Draw((Texture*)texture, Point2u(x, y), Point2u(w, h));
}
...
{
	render->_Render.Draw((Texture*)texture, Point2u(dx, dy), Point2u(x, y), Point2u(w, h));
}