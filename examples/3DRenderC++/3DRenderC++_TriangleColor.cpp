...
int main()
{
	...
	LDL::Window       window(result, context, LDL::Vec2i(0, 0), LDL::Vec2i((int)width, (int)height), "LDL 3D Render - Triangle color", LDL::Window::Resized);
	...
	while (window.IsRunning() && result.IsOk())
	{
		...
		rotationAngle += 0.02f;
		translationX = sin(rotationAngle) * 1.5f;
