...
const float TERRAIN_REPEAT = 25.0f;

// Параметры камеры с ограничениями
struct Camera
{
    float radius;
    float minRadius;
...
// Функции для работы с клавишами (простые обертки)
bool isKeyPressed(const LDL::Event& event, int key)
{
    return LDL_EventIsKeyPressed((LDL_Event*)&event, key);
}
...
int main()
{
    ...
    LDL::Window  window = LDL::Window(result, context, LDL::Vec2i(0, 0), LDL::Vec2i(800, 600),
        "LDL - Flying camera with zoom over terrain", LDL_WindowModeResized);
    ...
    if (result.IsOk())
    {
        ...
        while (window.IsRunning() && result.IsOk())
        {
            ...
            while (window.GetEvent(event))
            {
                ...
                if (event.Type == LDL_EventIsKeyboard && event.u.Keyboard.State == LDL_ButtonStatePressed)
                {
                ...
                if (event.Type == LDL_EventIsKeyboard && event.u.Keyboard.State == LDL_ButtonStateReleased)
                {
                    if (LDL_EventIsKeyPressed(&event, LDL_KeyW))
                        keyWPressed = false;
                    if (LDL_EventIsKeyPressed(&event, LDL_KeyS))
                        keySPressed = false;
                    if (LDL_EventIsKeyPressed(&event, LDL_KeyUp))
                        keyUpPressed = false;
                    if (LDL_EventIsKeyPressed(&event, LDL_KeyDown))
                        keyDownPressed = false;
                }
            }

            // Обработка приближения/отдаления
            if (keyWPressed || keyUpPressed)
            {
