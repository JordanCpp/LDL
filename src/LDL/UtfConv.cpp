...
void LDL_UtfConverter::encodeUTF8Char(uint32_t code_point, LDL_PodVector<char>& output)
{
    if (code_point <= 0x7F) 
    {
...
bool LDL_UtfConverter::convertToUTF8(const LDL_PodVector<uint16_t>& utf16_data)
{
...
const LDL_PodVector<uint16_t>& LDL_UtfConverter::getUTF16Vector() const
{
    return utf16_buffer;
}

const LDL_PodVector<char>& LDL_UtfConverter::getUTF8Vector() const
{
    return utf8_buffer;
}
